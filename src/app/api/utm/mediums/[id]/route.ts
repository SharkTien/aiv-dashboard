...
export async function DELETE(
  ...
  try {
    ...
    const [rows] = await pool.query(
      "SELECT id FROM utm_mediums WHERE id = ?",
      [id]
    );
    ...
    // Delete the medium
    await pool.query("DELETE FROM utm_mediums WHERE id = ?", [id]);
    ...
    return NextResponse.json({ error: "Failed to delete UTM medium" }, { status: 500 });
  }
}
