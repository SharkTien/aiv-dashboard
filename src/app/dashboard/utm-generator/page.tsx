...
export default function UTMGeneratorPage() {
  ...
  const loadData = async () => {
    setLoading(true);
    try {
      const [entitiesRes, sourcesRes, mediumsRes, activeRes] = await Promise.all([
        fetch('/api/entities'),
  ...
  const loadLinks = async (page: number) => {
    try {
      ...
      const res = await fetch(`/api/utm/links?${params}`);
      if (res.ok) {
        const data = await res.json();
  ...
  return (
    <div className="p-6 space-y-6">
        ...
        <form onSubmit={handleCreateUTM} className="space-y-4">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              ...
              <select value={selectedEntity} onChange={(e) => setSelectedEntity(e.target.value ? Number(e.target.value) : "")} className="w-full h-11 rounded-lg ring-1 ring-black/15 dark:ring-white/15 px-4 bg-white dark:bg-gray-800/50 text-slate-900 dark:text-white focus:ring-2 focus:ring-sky-500/50 transition-all" required>
      ...
      {/* UTM Links Table (hidden if lastCreated is shown) */}
      {!lastCreated && (
      <div className="bg-white/60 dark:bg-gray-700/60 rounded-xl p-6 border border-gray-200/50 dark:border-gray-600/50">
        <div className="flex items-center justify-between mb-4">
          <h2 className="text-lg font-semibold text-gray-900 dark:text-white">Generated UTM Links</h2>
          {pagination && (
        ...
        {links.length === 0 ? (
          <div className="text-center py-8 text-gray-500 dark:text-gray-400">
            No UTM links found. Create your first UTM link above.
          </div>
        ) : (
          <>
            <div className="overflow-x-auto">
                  ...
                  {links.map((link) => {
                    ...
                    return (
                      <tr key={link.id} className="border-b border-gray-200/50 dark:border-gray-600/50 hover:bg-gray-50/50 dark:hover:bg-gray-600/50">
                        ...
                        <td className="py-3 px-4 text-sm text-gray-900 dark:text-white">{link.utm_name || '-'}</td>
                        <td className="py-3 px-4 text-sm">
                          <div className="max-w-xs truncate text-blue-600 dark:text-blue-400">
                            {utmUrl}
                          </div>
                        </td>
                        ...
                        <td className="py-3 px-4 text-sm">
                          <div className="flex items-center gap-2">
                            <button
                              ...
                              className="px-3 py-1 text-xs rounded-lg bg-blue-100 dark:bg-blue-900/30 hover:bg-blue-200 dark:hover:bg-blue-900/50 text-blue-700 dark:text-blue-300 font-medium transition-colors"
                            >
                              Copy URL
                            </button>
                            <button
                              onClick={() => handleDeleteLink(link.id)}
                              className="px-3 py-1 text-xs rounded-lg bg-red-100 dark:bg-red-900/30 hover:bg-red-200 dark:hover:bg-red-900/50 text-red-700 dark:text-red-300 font-medium transition-colors"
                            >
                              Delete
                            </button>
                          </div>
