...
type TabType = 'notifications' | 'allocation-requests' | 'general';
type GeneralSubTab = 'new_features' | 'guideline';
...
export default function NotificationsPage() {
  ...
  const [generalTab, setGeneralTab] = useState<GeneralSubTab>('new_features');
  const [generalItems, setGeneralItems] = useState<any[]>([]);
  const [generalEditor, setGeneralEditor] = useState<{ title: string; content_html: string; category: GeneralSubTab } | null>(null);
  ...
  async function loadGeneral() {
    try {
      const res = await fetch(`/api/general-notifications?limit=50&category=${generalTab}`, { cache: 'no-store' });
      if (res.ok) {
        const data = await res.json();
        setGeneralItems(data.items || []);
      }
    } catch (e) { console.error('Error loading general notifications:', e); }
  }

  async function saveGeneral() {
    if (!generalEditor) return;
    setSavingGeneral(true);
    try {
      const res = await fetch('/api/general-notifications', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify(generalEditor)
      });
      if (res.ok) {
          ...
          <>
            ...
            {activeTab === 'general' && (
              <div className="space-y-6">
                ...
                <div className="border-b border-gray-200 dark:border-gray-700">
                  <nav className="-mb-px flex space-x-8">
                    <button onClick={() => setGeneralTab('new_features')} className={`py-2 px-1 border-b-2 font-medium text-sm transition-colors ${generalTab==='new_features' ? 'border-sky-500 text-sky-600 dark:text-sky-400' : 'border-transparent text-gray-500 hover:text-gray-700 hover:border-gray-300 dark:text-gray-400 dark:hover:text-gray-300'}`}>New Features</button>
                    <button onClick={() => setGeneralTab('guideline')} className={`py-2 px-1 border-b-2 font-medium text-sm transition-colors ${generalTab==='guideline' ? 'border-sky-500 text-sky-600 dark:text-sky-400' : 'border-transparent text-gray-500 hover:text-gray-700 hover:border-gray-300 dark:text-gray-400 dark:hover:text-gray-300'}`}>Guideline</button>
                  </nav>
                </div>
                ...
                {user.role === 'admin' && (
                  <div className="p-4 rounded-lg border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800">
                    <div className="flex items-center justify-between mb-3">
                      <h3 className="text-sm font-semibold text-gray-900 dark:text-white">Post a general notification</h3>
                      {!generalEditor && (
                        <button onClick={() => setGeneralEditor({ title: '', content_html: '', category: generalTab })} className="px-3 py-1.5 text-xs rounded bg-sky-600 text-white">New</button>
                      )}
                    </div>
                    {generalEditor && (
                      <div className="space-y-3">
                        <div className="flex items-center gap-3">
                          ...
                          <select
                            value={generalEditor.category}
                            onChange={(e)=>setGeneralEditor({ ...generalEditor, category: e.target.value as any })}
                        ...
                        <input value={generalEditor.title} onChange={(e)=>setGeneralEditor({ ...generalEditor, title: e.target.value })} placeholder="Title" className="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded bg-white dark:bg-gray-700 text-gray-900 dark:text-white text-sm" />
                        <RichHtmlEditor
                          html={generalEditor.content_html}
                          onChange={(html)=>setGeneralEditor({ ...generalEditor, content_html: html })}
                          placeholder="Paste from Word/Docs here and keep formatting..."
                        />
                        <div className="flex items-center gap-2">
                          <button onClick={saveGeneral} disabled={savingGeneral} className="px-3 py-1.5 text-xs rounded bg-green-600 text-white disabled:opacity-50">{savingGeneral ? 'Saving...' : 'Publish'}</button>
                          <button onClick={()=>setGeneralEditor(null)} className="px-3 py-1.5 text-xs rounded bg-gray-200 dark:bg-gray-600 text-gray-800 dark:text-gray-100">Cancel</button>
                        </div>
                      </div>
                    )}
                  </div>
                ...
                <div className="grid gap-3">
                  {generalItems.length === 0 ? (
                    <div className="text-center py-12"><p className="text-gray-500 dark:text-gray-400">No items.</p></div>
                  ) : generalItems.map(item => (
                    <GeneralItem key={item.id} item={item} isAdmin={user.role==='admin'} onChanged={loadGeneral} />
                  ))}
                </div>
              </div>
            )}
...
function GeneralItem({ item, isAdmin, onChanged }: { item: any; isAdmin: boolean; onChanged: () => Promise<void> }) {
  const [open, setOpen] = useState(false);
  const [editing, setEditing] = useState(false);
  const [title, setTitle] = useState(item.title as string);
  const [contentHtml, setContentHtml] = useState(item.content_html as string);
  const [saving, setSaving] = useState(false);

  const save = async () => {
    setSaving(true);
    try {
      const res = await fetch(`/api/general-notifications/${item.id}`, { method: 'PUT', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ title, content_html: contentHtml }) });
  ...
  const del = async () => {
    ...
    const res = await fetch(`/api/general-notifications/${item.id}`, { method: 'DELETE' });
  ...
  return (
    <div className="p-4 rounded-lg border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800">
      <div className="flex items-start justify-between">
        ...
        <div className="flex items-center gap-2">
          <button onClick={() => setOpen(true)} className="text-xs text-sky-600 dark:text-sky-400">View</button>
          {isAdmin && (
            <>
              <button onClick={() => setEditing(v=>!v)} className="text-xs text-amber-600">{editing ? 'Cancel' : 'Edit'}</button>
              <button onClick={del} className="text-xs text-red-600">Delete</button>
            </>
          )}
        </div>
      </div>
      ...
      {editing && (
        <div className="mt-3 space-y-2">
          <input value={title} onChange={(e)=>setTitle(e.target.value)} className="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded bg-white dark:bg-gray-700 text-gray-900 dark:text-white text-sm" />
          <textarea value={contentHtml} onChange={(e)=>setContentHtml(e.target.value)} className="w-full h-40 px-3 py-2 border border-gray-300 dark:border-gray-600 rounded bg-white dark:bg-gray-700 text-gray-900 dark:text-white text-sm font-mono" />
          <button onClick={save} disabled={saving} className="px-3 py-1.5 text-xs rounded bg-green-600 text-white disabled:opacity-50">{saving ? 'Saving...' : 'Save'}</button>
        </div>
      )}

      {open && (
        <div className="fixed inset-0 z-[1000] bg-black/40 flex items-center justify-center" onClick={()=>setOpen(false)}>
          <div className="bg-white dark:bg-gray-800 rounded-lg shadow-xl max-w-2xl w-[92%] p-4" onClick={(e)=>e.stopPropagation()}>
            <div className="flex items-center justify-between mb-3">
              <h4 className="text-sm font-semibold text-gray-900 dark:text-white">{item.title}</h4>
              <button onClick={()=>setOpen(false)} className="text-xs px-2 py-1 rounded bg-gray-100 dark:bg-gray-700 text-gray-700 dark:text-gray-200">Close</button>
            </div>
            <div className="prose prose-sm max-w-none dark:prose-invert" dangerouslySetInnerHTML={{ __html: item.content_html }} />
          </div>
        </div>
      )}
    </div>
